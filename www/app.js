...
let state = {
  ...
  serverUrl: "http://localhost:3000" // Server URL predefinito
};
...
function initApp() {
  ...
  // Caricamento Server URL
  const storedServerUrl = localStorage.getItem("amazon_price_tracker_server_url");
  if (storedServerUrl) {
    state.serverUrl = storedServerUrl;
  }
...
function setupEventListeners() {
  ...
  if (serverUrlInput) {
    serverUrlInput.addEventListener("input", (e) => {
      ...
      state.serverUrl = urlVal || "http://localhost:3000";
      localStorage.setItem("amazon_price_tracker_server_url", state.serverUrl);
      
      // Sincronizza automaticamente le credenziali col nuovo server
      syncTelegramProducts();
    });
  }
  ...
  if (tgTokenInput) {
    tgTokenInput.addEventListener("input", (e) => {
      ...
      fetch(state.serverUrl + "/api/telegram/config", {
        method: "POST",
  ...
  if (btnTriggerBriefing) {
    btnTriggerBriefing.addEventListener("click", async () => {
      ...
      try {
        const response = await fetch(state.serverUrl + "/api/telegram/morning-briefing", {
          method: "POST",
          headers: {
...
async function verifyAmazonUrlAndAutocompila() {
  ...
  try {
    const response = await fetch(`${state.serverUrl}/api/scrape?url=${encodeURIComponent(url)}`);
...
async function syncRealAmazonPrices() {
  ...
  for (const prod of state.products) {
    ...
    if (prod.url.startsWith("https://www.amazon.it") || prod.url.startsWith("https://www.amazon.com")) {
      ...
      try {
        const response = await fetch(`${state.serverUrl}/api/scrape?url=${encodeURIComponent(prod.url)}`);
...
async function analyzeReviewsWithGemini() {
  ...
  try {
    const response = await fetch(state.serverUrl + "/api/gemini/analyze", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        "x-gemini-key": state.geminiKey || ""
...
async function sendChatBotMessage(message) {
  ...
  try {
    const response = await fetch(state.serverUrl + "/api/gemini/chat", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        "x-gemini-key": state.geminiKey || ""
...
async function runTelegramChatIdDetection(token, isAuto = false) {
  ...
  chatIdDetectionInterval = setInterval(async () => {
    attempts++;
    try {
      const response = await fetch(`${state.serverUrl}/api/telegram/detect-chatid?token=${encodeURIComponent(token)}`);
      if (!response.ok) throw new Error(`HTTP ${response.status}`);
...
async function syncTelegramProducts() {
  try {
    await fetch(state.serverUrl + "/api/telegram/sync", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ 
        products: state.products,
...
async function pushTelegramNotification(message, imageUrl = "") {
  ...
  try {
    const response = await fetch(state.serverUrl + "/api/telegram/send", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
